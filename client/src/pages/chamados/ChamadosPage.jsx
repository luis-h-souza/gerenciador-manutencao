    ...
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      >
        <div className="card text-center" style={{ padding: "20px" }}>
          ...
          >
            {detalhe.financeiro?.totalChamados} chamados abertos
          </p>
        </div>
        <div
        ...
        >
          ...
          >
            Prejuízo: {fmt(detalhe.financeiro?.mauUso?.valor)}
          </p>
        </div>
      </div>
...
function CorporativoRegioes({ onSelect, mes, ano }) {
  ...
  const regioesOrdenadas = [...regionalData].sort(
...
export default function ChamadosPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      chamadosService
        .listar({ ...filtros, ano, mes, regiao: regiaoSelecionada, limit: 100 })
        .then((r) => r.data),
  ...
  if (isMacro && !regiaoSelecionada) {
    return (
      <CorporativoRegioes
        onSelect={(r) => setSearchParams({ regiao: r, mes, ano })}
        mes={mes}
        ano={ano}
      />
    );
  ...
  if (isMacro && regiaoSelecionada) {
    return (
      <CorporativoRegiaoDetalhe
        ...
        mes={mes}
        ano={ano}
        onBack={() => setSearchParams({ mes, ano })}
      />
    );
  }
